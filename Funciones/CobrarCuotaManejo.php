    ...
    function CobrarCuotaManejoTarjetas(){
        ...
        if(!empty($con->query($sql0))){
            foreach ($con->query($sql0) as $fila) {
                ...
                if($monto>=$cuota){
                    ...
                    $sql4 = 'INSERT INTO mensajes (id_origen,id_destino,contenido) VALUES('.$idAhorros.','.$idAhorros.',"Se ha hecho un descuento de '.$cuota.' para pago de cuota de manejo por fin de mes.")';
        ...
        if(!empty($con->query($sql1))){
            foreach ($con->query($sql1) as $fila) {
                ...
                if($monto>=$cuota){
                    $sql3 = 'UPDATE cuenta_ahorros SET saldo='.intval($monto-$cuota).' WHERE id='.$idAhorros;
                    $sql4 = 'INSERT INTO mensajes (id_origen,id_destino,contenido) VALUES('.$idAhorros.','.$idTarjeta.',"Se ha hecho un descuento de '.$cuota.' para pago de cuota de manejo por fin de mes.")';
